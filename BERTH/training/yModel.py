...
class HydroTrans(nn.Module):
    ...
    def forward(self, rs, mo, topo):
        ...
        mo_features = self.rs_encoder(self.mo_embedding(mo) + p_eb)
...
if __name__ == '__main__':
    model = HydroTrans(
        rs_dim=6, mo_dim=7, out_dim=4, max_len=500
    )
    model.forward(torch.randn((7, 500, 6)), torch.randn((7, 500, 7)), torch.randn((7, 3)))