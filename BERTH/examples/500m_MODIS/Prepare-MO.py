...
if not os.path.exists(f'{region_name}_500m_MO'):
    os.mkdir(f'{region_name}_500m_MO')
for day_offset in range(500):
    url = ee.Image(preprocess_era5land(day_offset)).select(columns_mo).float().getDownloadURL(
    ...
    np.save(f'{region_name}_500m_MO/{region_name}_500m_MO_{day_offset}.npy', mo_data.view((np.float32, len(columns_mo))))
